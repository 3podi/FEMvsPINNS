...
def neumann_derivatives(params,xs,ys):
    u = lambda x, y: ANN(params, jnp.stack((x, y)))
...
def pde_residual(params, points):
    return jnp.mean(residual(lambda x, y: ANN(params, jnp.stack((x, y))), points[:, 0], points[:, 1])**2)


@partial(jax.jit, static_argnums=0)
def pde_true(analytic_sol,params, points):
    return jnp.mean((ANN(params, jnp.stack((points[:, 0], points[:, 1]), axis=1)) - analytic_sol(points[:, 0], points[:, 1]) )**2)

@jax.jit
def boundary_dirichlet(params, points): # u(x,0) = 0
    return jnp.mean((ANN(params, jnp.stack((points[:,0],jnp.zeros_like(points[:,1])), axis=1)))**2) 
...
def validation_step(params, val_points, neumann_derivatives):
    # Generate validation points
    val_domain_xs, val_boundary_xs = val_points
    
    # Compute validation loss
    loss_val = (pde_residual(params, val_domain_xs) + 
                boundary_dirichlet(params,val_boundary_xs) +
                boundary_neumann(neumann_derivatives,params,val_boundary_xs)
                )
...
def train_loop(params, adam, opt_state, num_epochs, val_points, n_patience, validate_every=10, lr_scheduler=None, neumann_derivatives=neumann_derivatives):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % validate_every == 0:
            loss_val = validation_step(params, val_points, neumann_derivatives)  # Compute validation loss
...
def main():
    ...
    #----------------------------------------------------
    architecture_list = [[20,1],[60,1],[20,20,1],[60,60,1],[20,20,20,1],[60,60,60,1]]#,[20,20,20,20,1],[60,60,60,60,1]]
    #----------------------------------------------------
    # Train PINN
    #----------------------------------------------------
    ...
    for feature in architecture_list:
        ...
        for _ in range(1):
            ...
            u_approx = ANN(params, jnp.stack((domain_points[:,0], domain_points[:,1]),axis=1)).squeeze()
